...
class Collection extends React.Component{
	...
	fetchList = (lastTime,currentPage,pageSize,dir) => {
		let url = blogGlobal.requestBaseUrl + "/articles?mode=collection&userId=" + sessionStorage.getItem('uid') +"&lastTime="+lastTime+"&currentPage="+currentPage+"&pageSize="+pageSize+"&dir="+dir;
		fetch(url, {
			method: 'get',
			mode: 'cors',
			credentials: 'include',
			...
			if (status == 0) {
				...
				this.setState({ status: 1, collectionArticles: articleList ,pageTotal:pageTotal, lastTime:articleList[articleList.length - 1].createTime});
			}
		}).catch((err) => {
			console.log(err);
		});
	...
	render(){
		let {collectionArticles,status,pageSize,pageTotal,lastTime} = this.state;
