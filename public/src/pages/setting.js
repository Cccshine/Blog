...
class Setting extends React.Component {
		...
		render() {
				...
				let pwModalHtml = < div className = "form-box"
				styleName = "form-box-spc" >
					<
					form >
					<
				...
				placeholder = "确认密码"
				onChange = {
					this.handleComfirmPasswordChange
				}
				onFocus = {
					this.handleComfirmPasswordFocus
				}
				onBlur = {
					this.handleComfirmPasswordBlur
				}
				/> {
					comfirmPasswordStatus != 1 ? null : < TipBar type = "error"
					text = {
						comfirmPasswordTip
					}
					arrow = "has" / >
				...
				let emailModalHtmlStep1 = < div styleName = "safe-verify" >
					<
					p styleName = "tip" > 为了您的账号安全， 请通过密保邮箱 < br / > < strong > {
						email
					} < /strong>完成身份验证</p >
					<
				...
				styleName = "form-box-spc" >
					...
					<
					div className = "form-group"
				styleName = "form-group-double" >
					<
					input type = "text"
				name = "verify"
				placeholder = "邮箱验证码"
				onChange = {
					this.handleVerifyCodeChange
				}
				onFocus = {
					this.handleVerifyCodeFocus
				}
				/> <
				button className = "operate-btn"
				styleName = {
					verifyEmailStatus === 1 ? "disabled-btn" : ""
				}
				onClick = {
						this.sendVerify
					} > {
						...
						resetEmailStatus == 0 ? null : < TipBar type = 'error'
						text = {
							resetEmailTip
						}
						arrow = "has" / >
					...
					/div> <
					/div>;
				let emailModalHtmlStep2 = < div className = "form-box"
				styleName = "form-box-spc" >
					<
					form >
					<
				...
				placeholder = "新邮箱"
				onChange = {
					this.handleResetEmailChange
				}
				onFocus = {
					this.handleResetEmailFocus
				}
				onBlur = {
					this.handleResetEmailBlur
				}
				/> {
					resetEmailStatus == 0 ? null : < TipBar type = {
						resetEmailStatus == 1 ? 'error' : 'success'
					}
					text = {
						resetEmailTip
					}
				...
				return ( <
						...
						/ul> {
							pwModalShow ? < Modal { ...pwModalProps
							}
							/> : null} {
								emailModalShow ? < Modal { ...emailModalProps
								}
								/> : null} {
									showTip ? < TipBar { ...tipProps
									}
									/> : null} <
									/div>
								)
						...
						export default CSSModules(Setting, style, {
