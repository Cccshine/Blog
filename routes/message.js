...
router.get('/',(req,res) => {
	...
	if(currentPage === 0){//第一页
		MessageModel.find({receiveUser:req.session.uid}).then((messageList) => {//第一页时更新总数及总页数
			...
			// MessageModel.find({receiveUser:req.session.uid}).sort({createTime:-1}).limit(pageSize).populate('operateUser').populate('article').populate('comment').then((messageList) => {
			// 	if(messageList.length > 0){
			// 		return res.json({"status":1,"messageList":messageList,"pageTotal":messagePageTotal,"msg":"get success"});
			// 	}else{
			// 		return res.json({"status":0,"messageList":messageList,"pageTotal":messagePageTotal,"msg":"no message"});
		...
		MessageModel.find({receiveUser:req.session.uid}).sort({createTime:1}).limit(limit).populate('operateUser').populate('article').populate('comment').then((messageList) => {
			if(messageList.length > 0){
				return res.json({"status":1,"messageList":messageList,"pageTotal":messagePageTotal,"msg":"get success"});
			}else{
				return res.json({"status":0,"messageList":messageList,"pageTotal":messagePageTotal,"msg":"no message"});
		...
		MessageModel.find({receiveUser:req.session.uid,createTime:{$lt:lastTime}}).sort({createTime:-1}).limit(pageSize).populate('operateUser').populate('article').populate('comment').then((messageList) => {
			if(messageList.length > 0){
				return res.json({"status":1,"messageList":messageList,"pageTotal":messagePageTotal,"msg":"get success"});
			}else{
				return res.json({"status":0,"messageList":messageList,"pageTotal":messagePageTotal,"msg":"no message"});
		...
		MessageModel.find({receiveUser:req.session.uid,createTime:{$gt:lastTime}}).sort({createTime:1}).skip(skipNum).limit(pageSize).populate('operateUser').populate('article').populate('comment').then((messageList) => {
			messageList.reverse();
			if(messageList.length > 0){
				return res.json({"status":1,"messageList":messageList,"pageTotal":messagePageTotal,"msg":"get success"});
			}else{
