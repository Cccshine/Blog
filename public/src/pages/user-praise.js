...
class Praise extends React.Component{
	...
	componentWillMount = () => {
		let url = blogGlobal.requestBaseUrl + "/articles?mode=praise&userId=" + sessionStorage.getItem('uid');
		fetch(url, {
			method: 'get',
			mode: 'cors',
			credentials: 'include',
			...
			console.log(json);
			let { praiseArticles } = json;
			this.setState({ praiseArticles: praiseArticles});
		}).catch((err) => {
			console.log(err);
		});
	}
